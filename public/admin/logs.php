    ...
    <div class="container">
        ...
        <div class="log-tabs">
            <?php foreach ($log_configs as $key => $config): ?>
                <a href="?log=<?= $key ?>&lines=<?= $lines_count ?>&search=<?= urlencode($search_term) ?><?= $auto_refresh ? '&auto_refresh=1' : '' ?>" 
            ...
            <form method="GET" class="controls-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="lines" class="form-control">
                        <option value="50" <?= $lines_count === 50 ? 'selected' : '' ?>>50</option>
                        <option value="100" <?= $lines_count === 100 ? 'selected' : '' ?>>100</option>
                        <option value="200" <?= $lines_count === 200 ? 'selected' : '' ?>>200</option>
                        <option value="500" <?= $lines_count === 500 ? 'selected' : '' ?>>500</option>
                        <option value="1000" <?= $lines_count === 1000 ? 'selected' : '' ?>>1000</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search_term) ?>" 
                           class="form-control" placeholder="Terme à rechercher...">
                </div>
                ...
                <div class="form-group">
                    <label>&nbsp;</label>
                    <a href="?log=<?= $selected_log ?>&lines=<?= $lines_count ?>&search=<?= urlencode($search_term) ?>&auto_refresh=<?= $auto_refresh ? '0' : '1' ?>" 
                       class="btn <?= $auto_refresh ? 'btn-warning' : 'btn-success' ?>">
                        <?= $auto_refresh ? '⏸️ Arrêter' : '🔄 Auto-refresh' ?>
                    </a>
                </div>
            </form>
        </div>
        ...
        <!-- Statistiques -->
        <?php if (isset($log_data['lines'])): ?>
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value"><?= number_format(count($log_data['lines'])) ?></div>
                <div class="stat-label">Lignes affichées</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?= number_format($log_data['total_lines'] ?? 0) ?></div>
                <div class="stat-label">Total dans le fichier</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?= round(($log_data['total_size'] ?? 0) / 1024 / 1024, 2) ?> MB</div>
                <div class="stat-label">Taille du fichier</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?= isset($log_data['file_modified']) ? date('H:i:s', $log_data['file_modified']) : 'N/A' ?></div>
        ...
        <div class="log-container">
            <div class="log-header">
                <div class="log-info">
                    <strong><?= $current_log_config['icon'] ?> <?= htmlspecialchars($current_log_config['name']) ?></strong>
                    <?php if ($log_file_path): ?>
                        <br>Fichier: <code><?= htmlspecialchars($log_file_path) ?></code>
                    <?php endif; ?>
                </div>
                <div>
                    <a href="?log=<?= $selected_log ?>&lines=<?= $lines_count ?>&search=<?= urlencode($search_term) ?><?= $auto_refresh ? '&auto_refresh=1' : '' ?>" 
                       class="btn btn-secondary">🔄 Actualiser</a>
                </div>
            </div>
            ...
            <div class="log-content">
                <?php if (isset($log_data['error'])): ?>
                    <div class="error-message">
                        ❌ <?= htmlspecialchars($log_data['error']) ?>
                        
                        <h4>Chemins recherchés :</h4>
                        <ul>
                            <?php foreach ($current_log_config['paths'] as $path): ?>
                                <li><code><?= htmlspecialchars($path) ?></code> 
                ...
                <?php elseif (empty($log_data['lines'])): ?>
                    <div class="error-message">
                        ℹ️ Aucune ligne trouvée avec les critères actuels.
                    </div>
                <?php else: ?>
                    <?php foreach ($log_data['lines'] as $line): ?>
                        <div class="log-line <?= htmlspecialchars($line['level']) ?>">
                            <span class="log-line-number"><?= $line['number'] ?></span>
                            <?php if ($line['timestamp']): ?>
                                <span class="log-line-timestamp">[<?= htmlspecialchars($line['timestamp']) ?>]</span>
                            <?php endif; ?>
