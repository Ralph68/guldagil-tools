<?php
/**
 * Titre: AuthManager Sécurisé - Boucle redirection corrigée
...
class AuthManager {
    ...
    const SESSION_TIMEOUT = 34200; // 9h30
    ...
    /**
     * Vérification authentification - CORRIGÉ
     */
    public function isAuthenticated() {
        ...
        // 4. Mise à jour activité
        $this->updateLastActivity();
        return true;
    }
    
    /**
     * Session PHP valide - CORRIGÉ
     */
    private function isSessionValid() {
        ...
        // Vérifier expiration seulement si définie
    ...
    private function createUserSession($user, $remember) {
        ...
        // BDD
        $stmt = $this->db->prepare("INSERT INTO auth_sessions (id, user_id, expires_at) VALUES (?, ?, ?)");
        $stmt->execute([$session_id, $user['id'], $expires_at]);
        
        // Session PHP - TOUTES les variables
        $_SESSION['auth_session_id'] = $session_id;
        $_SESSION['auth_user_id'] = $user['id'];
        $_SESSION['auth_expires'] = time() + self::SESSION_TIMEOUT; // CRITIQUE
        $_SESSION['auth_user_data'] = [
            'id' => $user['id'],
            'username' => $user['username'],
            'role' => $user['role']
        ...
        // Compatibilité header.php
        $_SESSION['user'] = $_SESSION['auth_user_data'];
    ...
    private function createRememberToken($user_id) {
        ...
        $stmt = $this->db->prepare("UPDATE auth_users SET remember_token = ?, remember_expires = ? WHERE id = ?");
    ...
    private function recordFailedAttempt($username) {
        ...
        $stmt = $this->db->prepare("INSERT INTO auth_login_attempts (username, ip_address, user_agent) VALUES (?, ?, ?)");
        $stmt->execute([$username, $ip, $user_agent]);
    }
    
    private function clearFailedAttempts($username) {
        $stmt = $this->db->prepare("DELETE FROM auth_login_attempts WHERE username = ?");
        $stmt->execute([$username]);
    }
    ...
    public function logout($reason = 'manual') {
        if (isset($_SESSION['auth_session_id'])) {
            ...
            $this->logActivity('LOGOUT', $_SESSION['auth_user_data']['username'] ?? 'unknown', ['reason' => $reason]);
        }
    ...
    private function updateLastActivity() {
        if (isset($_SESSION['auth_session_id'])) {
            $stmt = $this->db->prepare("UPDATE auth_sessions SET expires_at = DATE_ADD(NOW(), INTERVAL ? SECOND) WHERE id = ?");
    ...
    private function cleanExpiredSessions($user_id = null) {
        if ($user_id) {
            $stmt = $this->db->prepare("DELETE FROM auth_sessions WHERE user_id = ? AND id != ?");
